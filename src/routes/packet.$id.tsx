        ...
        <article className="packet mx-auto max-w-[54rem] bg-packet px-5 py-9 text-packet-ink shadow-[0_40px_120px_-40px_rgba(0,0,0,0.35)] sm:px-14 sm:py-16 print:max-w-none print:shadow-none">
          ...
          <section>
            ...
            {overdue && (
              <p className="mt-5 border border-packet-ink/40 px-3 py-2 text-sm font-medium leading-relaxed">
                Re-read overdue since {d.nextReviewAt}. Re-read the official source before you treat
                this brief as current.
              </p>
            )}
          ...
          <section>
            ...
            <p className="mt-2 max-w-xl text-xs leading-relaxed text-packet-muted">
              Tailored to this water{job ? ` and to a ${jobLabel.toLowerCase()} day` : ""}. Every
              line is an action you take, not a condition we claim to know.
            </p>
              ...
              {CHECK_GROUPS.map((g) => {
                ...
                return (
                  <div key={g} className="break-inside-avoid">
                    <p className="font-display text-sm font-bold uppercase tracking-[0.1em]">{g}</p>
          ...
          <section className="break-inside-avoid">
            ...
            <p className="mt-3 text-[0.72rem] leading-relaxed text-packet-muted">
              An amenity that is not listed is one the source did not publish, not one that is
              absent. Gate hours, fees and same-day closures are set locally and are not mirrored
              here.
            </p>
          </section>
          ...
          <section>
            ...
            <p className="mt-2 text-[0.72rem] leading-relaxed text-packet-muted">
              Standing craft for this class of water, not an observation of this water today. No
              clarity, temperature, level, flow, tide or hatch is held here, and no spot is named.
            </p>
          ...
          <section className="break-inside-avoid">
            ...
            <div className="mt-5 divide-y divide-packet-rule border-y border-packet-rule">
              {layers.map((l) => (
                <div
                  key={l.key}
                  className="grid grid-cols-1 gap-1.5 py-4 sm:gap-2 sm:grid-cols-[10rem_1fr]"
                >
          ...
          <section className="grid grid-cols-1 gap-8 sm:grid-cols-2">
            ...
            <div>
              ...
              {datedWindows(d).length === 0 ? (
                <p className="mt-2 text-sm leading-relaxed">
                  No dated harvest closure published. Empty windows are a completed check, not a
                  gap. Do not assume harvest is open. Confirm on the official page.
                </p>
              ) : (
          ...
          <section data-print="hide" className="print:hidden">
            ...
            <p className="mt-2 text-[0.78rem] leading-relaxed text-packet-muted">
              Each link carries this record forward into the Hook instrument that answers the next
              question. Screen only — it is not part of the printed brief.
            </p>
            <ol className="mt-4 divide-y divide-packet-rule border-y border-packet-rule">
              {steps.map((s, i) => (
                <li key={s.id}>
                  <a href={s.url} className="flex min-h-12 items-center gap-3 py-3 text-packet-ink">
                    ...
                    <span aria-hidden="true" className="shrink-0 text-packet-muted">
                      ↗
                    </span>
                  </a>
                </li>
          ...
          <footer className="text-[0.72rem] leading-relaxed text-packet-muted">
            ...
            <p className="mt-2">
              Public, named destinations only. This brief contains no private spots, no coordinates,
              no catch expectation, and no live gauge, flow, tide, weather or hatch data. Conditions
              and regulations change without notice; the official source above governs. If a check
              cannot be cleared, the correct answer is not to go.
            </p>
            {d.managingAgency && (
              <p className="mt-3">
