...
import { downloadText, runToCsv, useRunManager, type RunTarget } from "@/lib/run-manager";
    ...
    withIdentity(
      { path: "/pipeline" },
      {
        meta: [
          ...
          {
            name: "description",
            content: "Official station sources and when a reading cannot be confirmed.",
          },
          {
            property: "og:title",
            content: "How a water reading comes together · Field Sense Navigator",
          },
          {
...
function Pipeline() {
  ...
  const progress = run.planned === 0 ? 0 : Math.round((run.counts.probed / run.planned) * 100);
      ...
      <main id="content">
        <section className="border-b border-hairline bg-abyss">
          <div className="mx-auto max-w-7xl px-5 py-12 sm:px-8 md:py-16">
            ...
            <p className="mt-5 max-w-xl text-sm leading-relaxed text-muted-foreground">
              Official gauge stations are matched to each named water on a nightly schedule (USGS,
              NOAA CO-OPS, Water Survey of Canada, USBR, USACE, CA DWR CDEC). Every record is
              located, given a weather station, scanned for agency-page language, and offered a
              gauge. Interior-west, pinned, and NOAA CO-OPS gauges refresh every 10 minutes; the
              full set every 30. USBR is isolated so a timeout cannot stall the rest. Misses stay
        ...
        <section className="mx-auto max-w-7xl px-5 py-12 sm:px-8">
          ...
          <p className="mt-3 max-w-2xl text-xs leading-relaxed text-muted-foreground">
            Matched stations are a committed source list. Every record is run through the same four
            live layers — location, gauge, weather observation, agency page language. Live numbers
            come from the last published reading, then a direct agency pull if that snapshot is
            older than 45 minutes. A silent feed is a miss, not a skip. Observation time, not fetch
            time, decides whether a value is current (48 h stage/flow/weather, 7 d reservoir
            elevation). Last official values may be retained with their original observed time when
            a fetch times out or when the last official observation is older than that window.
          </p>
          <ul className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
            <li className="panel p-5">
              ...
              <p className="data mt-3 text-sm text-foreground">
                {bind.located ?? 0}/{bind.records ?? destinations.length} located ·{" "}
                {bind.nwsBound ?? 0} weather stations
              </p>
              ...
              <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
                Generated{" "}
                {pulse?.bindings.generatedAt
                  ? new Date(pulse.bindings.generatedAt).toUTCString()
                  : new Date(bindingsFile.generatedAt).toUTCString()}
                . Pinned matches win. Name and water-type must both align. No nearby gauge is
                substituted.
              </p>
            </li>
            <li className="panel p-5">
              ...
              <li key={c.id} className="panel p-5">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <GradeChip
                    ...
                    label={
                      c.severity === "clear" ? "Pass" : c.severity === "watch" ? "Watch" : "Flagged"
                    }
                  />
                </div>
        ...
        <section className="mx-auto max-w-7xl px-5 pb-12 sm:px-8">
          ...
          <div className="mt-4" aria-live="polite" role="status" aria-label="Run progress">
            <div
          ...
          {visible.length > 0 && (
            <ul className="mt-4 divide-y divide-hairline border border-hairline">
              {visible.map((r) => (
                <li
                  key={r.id}
                  className="grid grid-cols-1 gap-2 p-4 md:grid-cols-[minmax(0,1fr)_auto]"
                >
            ...
            <table className="w-full min-w-[34rem] border-collapse text-sm">
              <thead>
                <tr className="border-b border-hairline">
                  <th scope="col" className="tick px-4 py-3 text-left text-[0.55rem]">
                    State
                  </th>
                  <th scope="col" className="tick px-4 py-3 text-right text-[0.55rem]">
                    Records
                  </th>
                  <th scope="col" className="tick px-4 py-3 text-right text-[0.55rem]">
                    Median readiness
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.state} className="border-b border-hairline/60">
                    ...
                    <td className="data px-4 py-3 text-right text-foreground/90">
                      {r.medianReadiness}
                    </td>
                    <td
