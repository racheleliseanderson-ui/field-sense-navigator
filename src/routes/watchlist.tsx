...
function WatchlistPage() {
  ...
  const avg =
    bands.length > 0 ? Math.round(bands.reduce((a, b) => a + b.score, 0) / bands.length) : 0;
        ...
        <section className="border-b border-hairline bg-abyss">
          <div className="mx-auto max-w-7xl px-5 py-12 sm:px-8 md:py-16">
            ...
            <p className="mt-5 max-w-xl text-sm leading-relaxed text-muted-foreground">
              Saved on this device only. Nothing is uploaded, and the boundary is unchanged — public
              waters with published access, nothing else.
            </p>
            ...
            {records.length > 0 && (
              <div className="mt-8 flex flex-wrap items-center gap-3">
                ...
                <span className="text-xs text-muted-foreground">mean readiness {avg}/100</span>
                <button
                  type="button"
                  onClick={exportAll}
                  disabled={busy}
